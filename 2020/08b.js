...
for (let i = 0; i < instructList.length; i++) {
  // console.log(i, instructList[i]);

  // swap operations 'jmp' and 'nop'
  if (instructList[i].includes('jmp')) {
    instructList[i] = instructList[i].replace('jmp', 'nop');
  } else if (instructList[i].includes('nop')) {
    instructList[i] = instructList[i].replace('nop', 'jmp');
  }

  // setup for fresh run through instruction list
  ...
  while (noRepeats == true) {
    ...
    // instruction will be undefined if it reaches the end of the program successfully
    if (!instruction) {
