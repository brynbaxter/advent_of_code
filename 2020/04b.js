...
const listToObjectArray = (passport) => {
...
const checkBYR = (byr) => {
  if (byr >= 1920 && byr <= 2002 && byr.toString().length == 4) {
    return true;
  } else {
    return false;
  }
...
const checkIYR = (iyr) => {
  if (iyr >= 2010 && iyr <= 2020 && iyr.toString().length == 4) {
    return true;
  } else {
    return false;
  }
...
const checkEYR = (eyr) => {
  if (eyr >= 2020 && eyr <= 2030 && eyr.toString().length == 4) {
    return true;
  } else {
    return false;
  }
...
const checkHGT = (hgt) => {
  if (hgt.match(/^\d+cm$/)) {
    let height = hgt.split('cm')[0];
    if (height >= 150 && height <= 193) {
      return true;
    }
  } else if (hgt.match(/^\d+in$/)) {
    let height = hgt.split('in')[0];
...
let arr = text.split('\n\n');
objectList = arr.map(listToObjectArray);

let validPassports = 0;
for (let i = 0; i < objectList.length; i++) {
  const passportObject = objectList[i];
  // thanks stackoverflow
  if (
    neededKeys.every((key) => Object.keys(passportObject).includes(key)) &&
    checkBYR(passportObject.byr) &&
    checkIYR(passportObject.iyr) &&
    checkEYR(passportObject.eyr) &&
    checkHGT(passportObject.hgt) &&
    checkHCL(passportObject.hcl) &&
    checkECL(passportObject.ecl) &&
    checkPID(passportObject.pid)
  ) {
    validPassports++;
  }
}
